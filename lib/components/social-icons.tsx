...
const TwitterIcon = ({ href, target = '_blank', className, size = 'md', share }: IconProps) => {
  ...
  return (
    <Link href={shareUrl || '#'} target={target} className={cn('text-footer-foreground hover:text-accent', className)}>
      <svg
...
const GithubIcon = ({ href = '#', target = '_blank', className, size = 'md' }: IconProps) => {
  return (
    <Link href={href} target={target} className={cn('text-footer-foreground hover:text-accent', className)}>
      <svg
        className={cn('w-6 h-6', size === 'sm' ? 'w-4 h-4' : size === 'lg' ? 'w-8 h-8' : '')}
        fill="currentColor"
        viewBox="0 0 24 24"
