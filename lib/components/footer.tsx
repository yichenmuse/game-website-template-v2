...
export default function Footer() {
  ...
  const buildLocaleLinks = (href: string, name: string) => {
    if (href === 'en') {
      ...
      return (
        <NextLink className="w-1/7 text-gray-500 hover:text-primary-200" href={pathname}>
          {name}
        </NextLink>
      );
    } else {
      return (
        <Link className="w-1/7 text-gray-500 hover:text-primary-200" href={pathname} locale={href}>
          {name}
        </Link>
      );
    }
        ...
        <div className="w-full max-w-[1480px] mx-auto items-center gap-16 ">
          <div className="flex flex-col gap-5 md:flex-row">
            <div className="flex flex-col justify-start gap-y-2 ">
              <div className="">
                <p className="text-gray-200 text-lg font-semibold sm:text-2xl font-leckerli">{t('title')}</p>
          ...
          <div className="mt-8">
            <div className="grid grid-cols-3 md:grid-cols-8 gap-2">
              ...
              {Object.entries(localeNames).map(([key, name]) => (
                <div key={key}>
                  {locale === key ? (
                    <span className="text-primary-200 w-1/7 cursor-default">{name}</span>
                  ) : (
